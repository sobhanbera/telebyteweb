	...
	return (
		<div className={styles.ProfileCardRightExpertise}>
			...
			<div className={styles.ExpertiseContainer}>
				{componentList ? componentList : "no expertise provided"}
			</div>
		</div>
	);
};
...
class OthersProfile extends Component {
	...
	componentDidMount() {
		...
		axios
			.get(
				"https://telebyte-new-default-rtdb.firebaseio.com/Users/" +
	...
	render() {
		if (this.state.notFound) {
			return (
				<ErrorSection
					code="User not found"
					message={`It seems that the user named (${
						window.location.pathname.split("/")[2]
					}) is not listed in the server.`}
				/>
			);
		} else if (this.state.cannotLoadData) {
			return (
				<ErrorSection
					code="User not found"
					message={`Some Error Occurred while retrieving user data. Please refresh the page. Or if this error is shown many time then please contact the developer.`}
				/>
			);
		}

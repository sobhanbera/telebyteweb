	...
	reloadUserAuthFuntion = () => {
		...
		firebase.auth().onAuthStateChanged(user => {
			if (user) {
				...
				// not registered
				console.log('You are not logged in please log in or sign up to continue.');
	...
	componentDidMount() {
		...
		setTimeout(() => {
			if (this.state.showLoading) {
				alert('Internet issues found. Please check your internet connection.');
			}
		}, 12000);
				...
				<Route
					...
					component={() => (
						<div className={styles.MainAppStarting}>
							...
							<Header signOutUser={this.signOutUser} loggedIn={this.state.loggedIn} />
